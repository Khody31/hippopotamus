...
QPoint coordinates_helpers::GameToWidgetCoord(const QVector2D& coord,
                                              const QVector2D& scene_size) {
  QVector2D inverted(coord.x(), -coord.y());
  QVector2D result =
      (inverted + game_constants::kMaxGameCoordinates) * scene_size
          / (2 * game_constants::kMaxGameCoordinates);
  return {static_cast<int>(result.x()), static_cast<int>(result.y())};
}

QVector2D coordinates_helpers::WidgetToGameCoord(const QPoint& coord,
                                                 const QVector2D& scene_size) {
  QVector2D result =
